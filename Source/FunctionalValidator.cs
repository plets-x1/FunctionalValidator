...
namespace Plets.Data.Xmi.FunctionalValidator {
        public class FunctionalValidator : Validator {
            ...
            foreach (UmlActionState s in diagram.UmlObjects.OfType<UmlActionState>())
            {
                ...
                {
                    if (s.TaggedValues.Keys.Contains("
                                cycles "))
                    {
                        String aux = s.GetTaggedValue("
                                jude.hyperlink ");

                        if (s.TaggedValues.Count - 1 > 1)
                        {
                            log(" [WARNING] Invalid tagged values at { " + HttpUtility.UrlDecode(s.Name) + " }.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                        }
                    ...
                    else if (!s.TaggedValues.Keys.Contains("
                                jude.hyperlink "))
                    {
                        log(" [WARNING] Invalid tagged values at { " + HttpUtility.UrlDecode(s.Name) + " }.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
        ...
        private void ValidateFinalStateCount(UmlDiagram diagram)
        {
            ...
            {
                case 0:
                    log(" [ERROR] Missing Final State
                                for { " + HttpUtility.UrlDecode(diagram.Name) + " } diagram.
                                ", 3);
                    break;
                case 1: //default. no statement.
                    break;
                default:
                    log(" [ERROR] Duplicated Final State found in { " + HttpUtility.UrlDecode(diagram.Name) + " } diagram.
                                ", 3);
                    break;
            }
        }
        ...
        private void ValidateInitialStateCount(UmlDiagram diagram)
        {
            ...
            {
                case 0:
                    log(" [ERROR] Missing Initial State
                                for { " + HttpUtility.UrlDecode(diagram.Name) + " } diagram.
                                ", 3);
                    break;
                case 1: //default. no statement.
                    break;
                default:
                    log(" [ERROR] Duplicated Initial State found in { " + HttpUtility.UrlDecode(diagram.Name) + " } diagram.
                                ", 3);
                    break;
            }
        }
        ...
        private void ValidateTransition(UmlDiagram diagram)
        {
            ...
            foreach (UmlTransition transition in diagram.UmlObjects.OfType<UmlTransition>())
            {
                ...
                {
                    log(" [ERROR] Missing target of Transition { " + HttpUtility.UrlDecode(transition.Name) + " }.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 3);
                }
                ...
                {
                    log(" [ERROR] Missing source of Transition { " + HttpUtility.UrlDecode(transition.Name) + " }.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 3);
                }
                ...
                if (transition.Target is UmlPseudoState)
                {
                    ...
                    {
                        log(" [WARNING] Transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " } points to pseudo - state and may not be tagged.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                    }
                }
                //transition pointing to linked states cannot be tagged
                else if (transition.Target is UmlActionState && !String.IsNullOrEmpty(transition.Target.GetTaggedValue("
                                jude.hyperlink ")))
                {
                    if (transition.TaggedValues.Count > 0)
                    {
                        log(" [WARNING] Transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " } points to linked state and may not be tagged.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                    }
                }
                ...
                else if (transition.Target is UmlInitialState)
                {
                    if (transition.TaggedValues.Count > 0)
                    {
                        log(" [WARNING] Transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " } points to initial state.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                    }
                }
        ...
        {
            String[] validTagNames = { "
                                TDACTION ", "
                                TDEXPECTEDRESULT " };
            String[] mandatoryTagNames = { "
                                TDACTION " };
            String[] optionalTag = { "
                                TDEXPECTEDRESULT " };
            ...
            foreach (KeyValuePair<String, String> tagvalue in transition.TaggedValues)
            {
                ...
                {
                    log(" [WARNING] Unexpected tag { " + tagvalue.Key + " } tagged in transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " }.Found at { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                }
            ...
            foreach (String tagvalue in mandatoryTagNames)
            {
                ...
                if (value == null)
                {
                    log(" [ERROR] Missing TDaction in { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " }.Found at diagram { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 3);
                }
                else
                {
                    ...
                    switch (tagvalue)
                    {
                        case "
                                TDACTION ":
                            if (value.Length < 1)
                                log(" [ERROR] Tag { TDaction } has no valid value
                                for transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " }.Found at diagram { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 3);
                            break;
                    }
                }
            ...
            foreach (String tagvalue in optionalTag)
            {
                ...
                if (value == null)
                {
                    log(" [WARNING] Missing TDexpectedResult in { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " }.Found at diagram { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
                }
                else
                {
                    ...
                    switch (tagvalue)
                    {
                        case "
                                TDEXPECTEDRESULT ":
                            if (value.Length < 1)
                                log(" [WARNING] Tag { TDexpectedResult } has no valid value
                                for transition { " + HttpUtility.UrlDecode(transition.Source.Name + " - > " + transition.Target.Name) + " }.Found at diagram { " + HttpUtility.UrlDecode(diagram.Name) + " }.
                                ", 2);
        ...
        private void ValidateDuplicatedActivitiesRecCall(UmlActivityDiagram actDiagram, UmlModel model, List<String> listActionForSingleUseCase, UmlUseCase useCase)
        {
            ...
            {
                if (!String.IsNullOrEmpty(act.GetTaggedValue("
                                jude.hyperlink ")))
                {
                ...
                else
                {
                    if (!(act is UmlInitialState || act is UmlFinalState))
                    {
                        ...
                        else
                        {
                            ...
                            {
                                log(" [ERROR] Action state { " + HttpUtility.UrlDecode(act.Name) + " } has its name duplicated.Found at { " + HttpUtility.UrlDecode(useCase.Name) + " } diagram.
                                ", 3);
        ...
        private Boolean HasCircularReference(Stack<UmlActivityDiagram> visitedDiagrams, UmlModel model, UmlActivityDiagram actDiagram)
        {
            ...
            {
                if (state.GetTaggedValue("
                                jude.hyperlink ") == null)
                {
                    continue;
                }

                UmlActivityDiagram referenced = model.Diagrams.Where(x => x is UmlActivityDiagram && x.Name.Equals(state.GetTaggedValue("
                                jude.hyperlink "))).FirstOrDefault() as UmlActivityDiagram;
                ...
                {
                    String temp = "
                                Circular reference found at { " + referenced.Name + " }.
                                ";
